...
export default function BookingPage() {
  ...
  const [createForm, setCreateForm] = useState(() => ({ fromDate: addDays(startOfToday(), 1), toDate: addDays(startOfToday(), 1), startMinutes: 600, endMinutes: 660, contactType: 'phone', contactValue: '', customerName: '', note: '' }))
  ...
  async function createReservation(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setCreateForm({ fromDate: addDays(startOfToday(), 1), toDate: addDays(startOfToday(), 1), startMinutes: 600, endMinutes: 660, contactType: 'phone', contactValue: '', customerName: '', note: '' })
      setShowCreate(false)
      setMessage(`已添加 ${created.length} 天预约记录`)
    } catch (createError) {
