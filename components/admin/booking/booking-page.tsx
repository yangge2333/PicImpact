...
type Settings = { id: string; bookingWindowDays: number; slotMinutes: number; schedules: Schedule[]; closedDates: string[] }
...
type CreateSelection = { id: number; date: string; startMinutes: number; endMinutes: number }
...
function initialCreateForm(): CreateForm {
  return {
    selections: [{ id: 1, date: addDays(startOfToday(), 1), startMinutes: 600, endMinutes: 660 }],
    contactType: 'phone',
    contactValue: '',
    customerName: '',
    note: '',
...
export default function BookingPage() {
  ...
  async function saveSettings() {
    ...
    setMessage('')
    try {
      await request('/api/v1/booking/settings', { method: 'PUT', body: JSON.stringify({ bookingWindowDays: settings.bookingWindowDays, schedules: settings.schedules, closedDates: settings.closedDates }) })
      setMessage('排班设置已保存')
    } catch (saveError) {
