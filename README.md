...
- 支持批量自动化上传，上传图片时会按管理端压缩配置生成预览图，以提供加载优化。
...
- 前端所有业务图片展示统一使用 `preview_url`，不再使用旧 OSS 派生图路径或派生图任务。
- 只有下载原图时才使用 `url` 原图地址。
- 管理端上传图片时会先根据压缩配置生成预览图，再随图片元数据一起入库。
- 当前默认预览图配置为短边 `2160px`、质量 `0.8`，可在管理端配置中调整。
- 预览图已经完成压缩，业务展示侧禁用 Next/Image 二次优化代理，避免产生 `/_next/image?...&w=...&q=...` 请求。

如需覆盖重生成已有图片的预览图，可在已配置数据库和对象存储环境变量的运行环境执行：

```shell
tsx scripts/regenerate-previews.ts
```

Docker 测试环境示例：

```shell
docker exec picimpact_v290 ./node_modules/.bin/tsx scripts/regenerate-previews.ts
```
...
- Preview-image based browsing with EXIF metadata and direct original downloads.
...
- Batch upload with configurable preview generation. The default preview config uses a 2160px short side and 0.8 quality.
...
- All business image displays use `preview_url`; legacy OSS derived image paths and derived image tasks are no longer used.
- Original image `url` is used only for downloads.
- Admin uploads generate the preview image according to the compression config before persisting image metadata.
- The default preview config uses a `2160px` short side and `0.8` quality, and can be adjusted in the admin settings.
- Preview images are already compressed, so display components bypass the Next/Image optimization proxy and avoid `/_next/image?...&w=...&q=...` requests.

To overwrite and regenerate existing preview images in an environment with database and object storage variables configured:

```shell
tsx scripts/regenerate-previews.ts
```

Docker test environment example:
...
- バッチアップロード、管理画面の圧縮設定に基づくプレビュー画像の自動生成。
